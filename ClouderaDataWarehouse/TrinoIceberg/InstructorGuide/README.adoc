...
Once the environment and the Keycloak servers are created, upload the raw csv file to the object store. This will be used to create tables during the workshop.
...
=== Modifying the Trino Policy in Ranger
To modify the Trino Policy to allow access to trino warehouse and allow permission to run queries for the end user, perform the following steps:

. Navigate to the Ranger UI.  
. Under the `Resource Policies` tab, click `cm_trino`.  

. Search for policy number 116 (all - trinouser), 125 (all - catalog, schema, table, column) and 123 (all - queryid) one by one, click the pencil icon on the right to edit. 
. In the **Allow Conditions** section, under `Select Groups`, search for and add the CDP group named <workshop_name>-aw-cdp-user-group.
+  
image::./media/all_trinouser.png[alt="Sample Policy Details Page in Ranger UI",940,420] 
image::./media/databse_column.png[alt="Sample Policy Details Page in Ranger UI",940,420]
image::./media/queryid_policy.png[alt="Sample Policy Details Page in Ranger UI",940,420] 
