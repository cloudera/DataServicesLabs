...
* A partitioned Apache Iceberg table (`iceberg.airline_lab.flights_<username>`) populated from a Kaggle airline dataset.
* Four reusable Python notebooks that query the table with both the native `trino-python-client` driver and with `SQLAlchemy`.
...
TIP: If you have less time, complete Lab 0 and Lab 1 and Lab 2, Labs 3 and Lab 4 can be revisited independently.
...
. Click on your user name at the top right corner then click on Profile..
+
image::./images/profile.png[alt="top navigation bar in CDP Console with Profile highlighted"]

. Click option Set Workload Password.
+
image::./images/set_workloadpwd.png[alt="Set Workload Password option"]

. Enter a password that you can remember. We will require this while connecting to Trino VW.
. Click the button `Set Workload Password`.
+
image::./images/provide_pwd.png[alt="Provide Workload Password"]
...
NOTE: The raw CSV is already staged in S3 in the bucket for this lab using below command.
...
The same DDL is also available in `scripts/01_ddl.sql` if you prefer to run it directly from the Cloudera Data Explorer. Run each step one at a time, confirming success before moving on.

=== Step 3 : Load data with Trino

Run the final cell of the notebook (`notebooks/01_data_setup.ipynb`) (or run `scripts/02_insert.sql`). The `INSERT INTO ... SELECT` copies and type-casts every row from the staging table into the Iceberg table. Iceberg produces columnar Parquet files grouped by the declared partitioning transforms.
...
[source,sql]
----
SELECT COUNT(*)            AS total_flights     FROM iceberg.airline_lab.flights_${username};
SELECT flight_status, COUNT(*) AS cnt           FROM iceberg.airline_lab.flights_${username} GROUP BY flight_status;
----
...
NOTE: The same DDL is also available in `scripts/03_queries.sql` which covers lab2 and lab3. if you prefer to run it directly from the Cloudera Data Explorer. Run each step one at a time, confirming success before moving on.
...
NOTE: The same DDL is also available in scripts/04_lab.sql which covers lab4. If you prefer to run it directly from the Cloudera Data Explorer. Run each step one at a time by updating placeholder values, confirming success before moving on.
