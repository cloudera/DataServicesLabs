...
-- Replace placeholder "<username>" with your username. (Press Ctrl + H (Windows/Linux) or Cmd + Option + F (Mac) directly in the Hue editor to replace all)
-- ============================================================
...
ALTER TABLE iceberg.airline_lab.flights_<username> ADD COLUMN loyalty_tier VARCHAR;

-- Verify the new column appears in the schema
DESCRIBE iceberg.airline_lab.flights_<username>;
...
FROM iceberg.airline_lab."flights_<username>$snapshots"
...
UPDATE iceberg.airline_lab.flights_<username>
...
CREATE OR REPLACE TABLE iceberg.airline_lab.flights_<username> AS
SELECT * FROM iceberg.airline_lab.flights_<username>
...
-- on: iceberg → airline_lab → flights_<username>
